...
@section('content')
  ...
  <h1 class="text-2x1 font-semibold leadind-tigh py-2">
          ...
          @foreach ($users as $user)
              <tr>
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                    @if ($user->image)
                        <img src="{{ url("storage/{$user->image}") }}" alt="{{ $user->name }}" class="object-cover w-14">
                    @else
                        <img src="{{ url("images/favicon.ico") }}" alt="{{ $user->name }}" class="object-cover w-14">
                    @endif
                    {{ $user->name }}
                </td>
